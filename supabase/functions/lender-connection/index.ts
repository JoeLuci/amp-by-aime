// Lender Connection Request Edge Function
// Handles submission of lender connection requests
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.lenderId) {
      return errorResponse('Missing required field: lenderId', 400)
    }
    ...
    try {
      const opportunityId = await ghlClient.createOpportunity({
        ...
        pipelineId: Deno.env.get('GHL_PIPELINE_LENDER_CONNECTION')!,
        stageId: Deno.env.get('GHL_STAGE_LENDER_CONNECTION')!,
        ...
        customFields: [
          ...
          { key: 'submission_type', field_value: 'Lender Connection' }
        ]
      })
      ...
      return successResponse({
        id: submission.id,
        opportunityId,
        message: 'Lender connection request submitted successfully'
      })
