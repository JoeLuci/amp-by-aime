-- Update the get_admin_dashboard_metrics function to return detailed engagement by plan tier
-- Shows views, connections, and unique users per plan tier
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE ae.event_type = 'contact' AND ae.content_type = 'vendor') as vendor_connections,
    COUNT(*) FILTER (WHERE ae.event_type = 'contact' AND ae.content_type = 'lender') as lender_connections,
    ...
    -- Engagement by plan tier (detailed breakdown by content type)
    (
