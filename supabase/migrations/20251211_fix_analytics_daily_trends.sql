-- Fix: Correlated subquery error in daily_trends section
-- Error was: subquery uses ungrouped column "ae2.created_at" from outer query
-- Solution: Use LEFT JOINs instead of correlated subqueries
...
BEGIN
  ...
  SELECT
    ...
    (
      ...
      FROM (
        SELECT
          COALESCE(p.plan_tier::text, 'Free') as plan_tier,
          COUNT(DISTINCT ae2.user_id) as unique_users,
          ...
          (SELECT COUNT(*) FROM vendor_connections vc
           ...
           WHERE vp.plan_tier::text = COALESCE(p.plan_tier::text, 'Free')
             AND vc.created_at >= p_start_date
             AND vc.created_at <= p_end_date) as vendor_connections,
          (SELECT COUNT(*) FROM lender_connections lc
           JOIN profiles lp ON lc.user_id = lp.id
           WHERE lp.plan_tier::text = COALESCE(p.plan_tier::text, 'Free')
             AND lc.created_at >= p_start_date
    ...
    -- Fixed: Use LEFT JOINs instead of correlated subqueries for daily connections
    (
      ...
      FROM (
        ...
        FROM (
          ...
          FROM analytics_events ae2
          WHERE ae2.created_at >= p_start_date AND ae2.created_at <= p_end_date
